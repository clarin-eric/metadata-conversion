...
#Identifiers of sets to retrieve records for
...
#Maximum number of sample records to retrieve per set
RECORDS_PER_SET=5
OAI_BASE_URL="http://oai.europeana.eu/oaicat/OAIHandler"
...
do
	#set
	echo "- Set: $SET_ID"
	URL="${OAI_BASE_URL}?verb=ListIdentifiers&metadataPrefix=edm&set=${SET_ID}"
	RECORD_IDS=`curl -s "$URL" \
		|egrep -o "<identifier>[^<]+</identifier>" \
		|head -n ${RECORDS_PER_SET} \
		|egrep -o "http[^<]+"`
	i=0
	...
	do
		...
		if [ "" != "$RECORD_ID" ]
			then
				...
				URL="${OAI_BASE_URL}?verb=GetRecord&metadataPrefix=edm&identifier=${RECORD_ID}"
				OAI_FILE="${OAI_DIR}/${SET_ID}-example-${i}.oai.xml"
				EDM_FILE="${EDM_DIR}/${SET_ID}-example-${i}.xml"
